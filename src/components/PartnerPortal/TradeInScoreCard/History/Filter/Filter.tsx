...
import { FilterStatus, RedBarnQuotesFilterStatus, TradeInFilterStages, TradeInFilterStatus } from './filterContaints';
...
const Filter: FC = () => {
  ...
  const defaultShow = useMemo(() => {
    return query.tab === constHistoryPartnerTradeInTabName.TRADE_IN_REQUEST;
  }, [query]);
  ...
  const renderFilterSelectStatus = useMemo(() => {
    if (defaultShow) {
      return TradeInFilterStatus;
    }
    if (isRedBarnQuote) {
      return RedBarnQuotesFilterStatus;
    }
    if (`${query?.tab}` === constHistoryPartnerTradeInTabName.INBOX) {
      return FilterStatus.filter(
        (it) => it.value !== ScorecardStatuses.QUOTE && it.value !== ScorecardStatuses.EXPIRED_QUOTE,
      );
    }
    return FilterStatus;
  }, [defaultShow, isRedBarnQuote, query]);

  const renderFilterSecteStages = useMemo(() => {
    if (defaultShow) {
      switch (query?.statuses) {
        case statusToTextHistoryTradeIn.NEW_LEAD:
          return [
        ...
        case statusToTextHistoryTradeIn.OPEN:
          return [
            { value: '', label: 'All Stages' },
            {
        ...
        case statusToTextHistoryTradeIn.CONVERTED:
          return [
            { value: '', label: 'All Stages' },
            {
        ...
        case statusToTextHistoryTradeIn.CLOSED:
          return [
            { value: '', label: 'All Stages' },
            {
  ...
  const hideFilterByDate = useMemo(() => {
    return (
      !query?.tab ||
      query?.tab === constHistoryPartnerTradeInTabName.INBOX ||
      query?.tab === constHistoryPartnerTradeInTabName.TRADE_IN_REQUEST
    );
  }, [query]);
  ...
  return (
    <>
      <Card className={cx('mt-3', classes.wrapCardFilter)}>
        ...
        {!isRedBarnQuote && visibleDropdownExport && (
