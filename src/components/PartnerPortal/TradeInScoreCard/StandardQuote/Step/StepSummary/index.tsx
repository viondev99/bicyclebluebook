...
const StepSummary: FC<Props> = ({
  ...
  useEffect(() => {
    if (detailScoreCard) {
      setStagesSelected(detailScoreCard?.stage);
      setStausSelected(detailScoreCard?.status);
      setNotes(detailScoreCard?.owner?.notes);
    }
  }, [detailScoreCard]);
  ...
  useEffect(() => {
    const checkTradeInComplete = async () => {
      ...
      if (statusToTextHistoryQuotes.OPEN_QUOTE === res.status) {
        setIsEnableButtonEdit(true);
      }
    };
    checkTradeInComplete().then(() => {});
  }, [router]);
  ...
  const handleChangeValueSelect = useCallback((option: Option, field) => {
    ...
    switch (option?.value) {
      ...
      // case StagesToTextHistoryQuotes?.EXPIRED:
      //   setStausSelected(statusToTextHistoryQuotes.EXPIRED_QUOTE);
      //   break;
  ...
  const isDisableStage = useMemo(() => {
    return detailScoreCard?.status !== statusToTextHistoryQuotes.OPEN_QUOTE;
  }, [detailScoreCard]);

  const renderFormSelectStage = useCallback(
    (field: string) => {
      switch (field) {
        case 'status':
          return (
            <Row className={classes.rowStatus}>
              <Col lg={1} md={2} xs={12}>
                <div>Status</div>
              </Col>
              <Col lg={6} md={8} xs={12} className={classes.customCol}>
                <Select
                  inputId={field}
                  isSearchable={true}
            ...
            <Row className={classes.rowStatus}>
              ...
              <Col lg={6} md={8} xs={12} className={classes.customCol}>
                <Select
                  ...
                  options={ListFilterStagesQuotes?.filter((item) => item?.value !== '')}
                  onChange={(value: Option) => handleChangeValueSelect(value, 'status')}
                  isDisabled={isDisableStage}
                />
              </Col>
            </Row>
          );
    ...
    [handleChangeValueSelect, isDisableStage, notes, stagesSelected, statusSelected],
  );
    ...
    (subStepFive?: boolean) => {
      if (subStepFive) {
        return router.push({
          ...
          query: { step: 1, editStepDetailOfQuote: true, subStepFive: true, isQuote: true },
        });
      }
      router.push({
        pathname: `/trade-in-account/trade-in/${router?.query?.id}`,
        query: { step: 1, editStepDetailOfQuote: true, isQuote: true },
      });
    },
    [router],
  );

  const isViewQuoteTab = useMemo(() => {
    return router?.query?.isQuote && router?.query?.isViewQuote;
  }, [router]);
  ...
  const renderConvertedQuote = useMemo(() => {
    return (
      isViewQuoteTab &&
      !isDisableStage && (
        <div className={classes.wrapTradeInValue}>
          <div className={classes.header}>
            <Button type="button" onClick={() => setVisibleModalConfirmGotoCreateScorecard(true)} disabled={false}>
              Convert quote to scorecard
            </Button>
          </div>
        </div>
      )
    );
  }, [isDisableStage, isViewQuoteTab]);
  ...
  const handleSaveQuote = useCallback(async () => {
    ...
    const payload: CreateNewTradeInBody = {
      bicycleId: detailScoreCard?.bicycleBaseInfo?.bicycleId,
      chargerIncluded: false,
      clean: null,
      compRequests: [{ compId: '178', value: detailScoreCard?.tradeInComponents[0]?.value }],
      condition: detailScoreCard?.condition,
      hasDiagnosticReport: false,
      hasKey: false,
      isEbike: false,
      isTamperedWith: false,
      upgradeCompIds: [],
      value: Number(detailScoreCard?.tradeValue),
      ownerNotes: notes,
    ...
    try {
      ...
      if (statusSelected !== statusToTextHistoryQuotes.OPEN_QUOTE) {
        setIsEnableButtonEdit(false);
      }
      toastSuccess('Updated Successfully');
      dispatch(scoreCardAction.getStepSummaryStandardQuote(`${router?.query?.id}`));
    } catch (error) {
      toastError(error);
    }
  }, [detailScoreCard, dispatch, notes, router, stagesSelected, statusSelected]);
  ...
  return (
    <>
                ...
                <div className={classes.wrapForm}>
                  <div>
                    ...
                    {isViewQuoteTab && (
                      <div className={classes.customRow}>
                        {renderFormSelectStage('status')}
                        {renderFormSelectStage('stages')}
                        {renderFormSelectStage('notes')}
                      </div>
                    )}
                  </div>
                  ...
                  {isViewQuoteTab && (
                    <div className={classes.btnSave}>
                      <Button type="button" onClick={handleSaveQuote} disabled={isDisableStage}>
                        Save Changes
                      </Button>
                    </div>
                  )}
      ...
      {!loadingBarcode && <PrintScoreCard dataFormStepSummary={dataPrint} />}
