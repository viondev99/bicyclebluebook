...
import { TREK_PARTNER_PARENT_ID } from 'helpers/utilities.helper';
...
const StepShipping: FC<Props> = ({
  ...
  useEffect(() => {
    if (detailPartner && reload) {
      if (
        detailPartner?._id === TREK_PARTNER_PARENT_ID ||
        detailPartner?.partner_parent?._id === TREK_PARTNER_PARENT_ID
      ) {
        setActiveShip(2);
      }
    }
