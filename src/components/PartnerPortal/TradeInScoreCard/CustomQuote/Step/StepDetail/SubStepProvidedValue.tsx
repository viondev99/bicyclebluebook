...
import {
  createNewTradeInCustomQuoteProvidedValueRequest,
...
const SubStepProvidedValue: FC<Props> = () => {
  ...
  const handleSubmitCreateNewTradeInCustomQuoteProvidedValue = useCallback(
    async (isInstantPayoutss: boolean) => {
      try {
        await createNewTradeInCustomQuoteProvidedValueRequest(
          {
            customQuoteId: `${dataDetailCustomQuote?.id}`,
          },
          { isInstantPayout: isInstantPayoutss },
        );
        router.push({
          pathname: `/trade-in-account/trade-in/${dataDetailCustomQuote?.tradeIn?.id}`,
          query: { step: 2 },
        });
      } catch (error) {
        toastError(error);
