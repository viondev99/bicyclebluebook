...
import {
  ...
  CompleteCustomQuoteRequest,
  CompleteCustomQuoteResponse,
  completeCustomQuoteUploadImageRequest,
  deleteCustomQuoteUploadImageRequest,
  saveCustomQuoteRequest,
  saveFirstCustomQuoteRequest,
...
const CustomQuote: FC<Props> = ({ isCreateNewCustomQuote }) => {
  ...
  const gotoStep = useCallback(
    (stepNumber: number, subStepNumber?: number) => {
      ...
      if (query?.id) {
        router.push({
          pathname: `/trade-in-account/trade-in/custom-quote/${query?.id}`,
          query: _query,
        });
      } else {
        router.push({
  ...
  const handleCheckCompleteStep = useCallback(async (id: string) => {
    const res: CheckTradeInCompleteStepResponse = await checkTradeInCompleteStepRequest(id);
    if (res?.status === ScorecardStatuses.CUSTOM_QUOTE_VALUE_PROVIDED) {
      setVisibleModalValueProvided(true);
    } else {
      setVisibleModalValueProvided(false);
    }
    const isComplete =
      res.status === ScorecardStatuses.COMPLETED ||
      // res.status === ScorecardStatuses.CUSTOM_QUOTE_PENDING_REVIEW ||
      res.status === ScorecardStatuses.CUSTOM_QUOTE_VALUE_PROVIDED;
  ...
  useEffect(() => {
    if (dataDetailCustomQuote && !isCreateNewCustomQuote) {
      ...
      setFormStepDetailSubStepOne({
        ...
        serialNumber: dataDetailCustomQuote?.tradeIn?.serialNumber || '',
      ...
      setOldImageStepThree(
        dataDetailCustomQuote?.tradeInImages?.length
          ? [...dataDetailCustomQuote?.tradeInImages].map((it: any) => {
              return {
                ...it,
                id: it?.id,
                url: it?.fullLink,
      ...
      // step contact
      setFormStepContact({
        employeeEmail: dataDetailCustomQuote?.employeeEmail || '',
        employeeLocation: dataDetailCustomQuote?.employeeLocation || '',
        employeeName: dataDetailCustomQuote?.employeeName || '',
        ownerEmail: dataDetailCustomQuote?.tradeIn?.ownerEmail || '',
        ownerName: dataDetailCustomQuote?.tradeIn?.ownerName || '',
        ownerPhone: dataDetailCustomQuote?.tradeIn?.ownerPhone || '',
      });
      handleCheckCompleteStep(`${dataDetailCustomQuote?.tradeIn?.id}`);
      setVisibleSaveStep(true);
    }
  }, [dataDetailCustomQuote, query]);

  const handleGetDefaultCustomQuote = useCallback(async () => {
    dispatch(scoreCardAction.getDetailCustomQuote(`${query?.id}`));
  }, [dispatch, query]);
  ...
  const handleUploadImageCustomQuote = useCallback(
    async (id: string, isDraft?: boolean) => {
      ...
      try {
        ...
        const idCustomQuote = {
          customQuoteId: Number(id),
        };
        if (listDeleted?.length === 0) {
          await completeCustomQuoteUploadImageRequest(idCustomQuote, formData, {
            isDraft,
          ...
          await deleteCustomQuoteUploadImageRequest(id, {
            isDraft,
            oldImageIdsNewOrder: listOldImageIdsNewOrder,
  ...
  const onSave = useCallback(async () => {
    if (visibleSaveStep) {
      try {
        ...
        let response: CompleteCustomQuoteResponse;
        if (!query?.id) {
          response = await saveFirstCustomQuoteRequest(payload);
        } else {
          await saveCustomQuoteRequest(`${query?.id}`, payload);
        }
        await handleUploadImageCustomQuote(`${response?.customQuoteId || query?.id}`, true);

        if (listDataStepCustomQuote.length > 1) {
          toastSuccess('Saved.');
          return router.push({
            pathname: `/trade-in-account/trade-in/custom-quote/${`${response?.customQuoteId || query?.id}`}`,
            query: {
              step,
              subStep,
            },
  ...
  const handleSubmitStepContact = useCallback(async () => {
    ...
    try {
      ...
      if (query?.id) {
        ...
        response = await completeCustomQuoteRequest(payload);
      }
      await handleUploadImageCustomQuote(query?.id ? `${query?.id}` : `${response?.customQuoteId}`);
      ...
      if (findFillData) {
        cloneArr[indexScorecardSelected] = {
          ...
          id: response?.customQuoteId || query?.id,
          indexStepComplete: 4,
          step: 4,
        };
      } else {
    ...
    <div className={cx(classes.tradeInLayout, classes.container)}>
      ...
      {/* {visibleModalValueProvided && (
        <ModalValueProvided
          ...
          onClose={() => setVisibleModalValueProvided(false)}
