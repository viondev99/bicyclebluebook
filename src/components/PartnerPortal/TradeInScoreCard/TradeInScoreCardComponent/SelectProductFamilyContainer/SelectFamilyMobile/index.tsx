...
const SelectFamilyMobile: FC<Props> = ({ families, familyName, handleSelectModel }) => {
  ...
  useEffect(() => {
    if (families.length > 0) {
      ...
      const familyNameChar = familyName.charAt(0);
      if (familyName !== '') {
        setFilteredAlphabet(familyNameChar);
        const filteredModelsByChar = families.filter((item: string) => {
          return _upperCase(item[0]) === _upperCase(familyNameChar);
        });
              ...
              <Row>
                {filteredModels.map((item: string) => (
                  <Col xs={12} key={item} className={classes.family} onClick={() => handleSelectModel(item)}>
                    <span className={cx(classes.name, familyName === item && classes.active)}>{item}</span>
