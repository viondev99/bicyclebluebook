...
const SubStepTwo: FC<Props> = ({
  ...
  useEffect(() => {
    dispatch(getBrandYearModelV2({ brandId: form.brand, familyName: form.familyName, isEbike: true }));
  ...
  useEffect(() => {
    ...
    if (dataBrandYearModel?.years?.length && isUpdateYear) {
      ...
      setListYear([...dataBrandYearModel?.years.filter((it: Year) => it.id >= 2018)]);
      if (formStepDetailSubStepTwo?.bicycleId) {
        const indexYear = dataBrandYearModel?.years?.findIndex((it) => it.id === formStepDetailSubStepTwo.yearId);
  ...
  const handleChangeYear = (yearId: number, index: number) => {
    ...
    dispatch(getBrandYearModelV2({ brandId: form.brand, familyName: form.familyName, yearId, isEbike: true }));
    setActiveYear(index);
  ...
  }, [handleSelectBicycle, listBicycles, listYear, selectedBicycleId]);
