...
import { constTitleStep } from 'components/PartnerPortal/TradeInScoreCard/CustomQuote/constraint';
import PrintScoreCardRedBarn from 'components/PartnerPortal/TradeInScoreCard/TradeInScoreCardComponent/PrintScoreCardRedBarn';
...
import { changeNameCondition, DATA_RESET_CREATE_SCORECARD_QUANTITY, printContent } from 'helpers/utilities.helper';
...
const ModalComplete = React.lazy(() => import('./ModalComplete'));

interface Props {
  handleSubmitStepSummary: (isSave: boolean, values: FormSummary) => Promise<boolean>;
...
const StepSummary: FC<Props> = ({
  ...
  const { detailScoreCard, detailPartner, dataDetailStandardQuote, dataDetailCustomQuote, isLoading } = useSelector(
    (store: StoreState) => ({
      ...
      dataDetailStandardQuote: store.partner.scorecard.dataDetailStandardQuote,
      dataDetailCustomQuote: store.partner.scorecard.dataDetailCustomQuote,
      isLoading: store.partner.scorecard.loading,
    }),
  );
  ...
  const dataStepSummary = useMemo(() => {
    return listDataStepStandardQuote[indexScorecardSelected]?.dataStepSummary;
  }, [indexScorecardSelected, listDataStepStandardQuote]);
  ...
  const initialValues: FormSummary = useMemo(() => {
    return {
      firstName: detailScoreCard?.owner?.firstName || detailScoreCard?.owner?.name || dataStepSummary?.firstName,
      ...
      proofDate:
        detailScoreCard?.proof?.date && moment(detailScoreCard?.proof?.date).isValid()
          ? new Date(detailScoreCard?.proof?.date)
          : dataStepSummary?.proofDate && moment(dataStepSummary?.proofDate).isValid()
          ? new Date(dataStepSummary?.proofDate)
          : new Date(),
      paypalEmail: '' || dataStepSummary?.paypalEmail,
      confirmEmail: '' || dataStepSummary?.confirmEmail,
    };
  ...
  const frameSize = useMemo(() => {
    ...
    if (isCustomQuote && dataDetailCustomQuote) {
      return dataDetailCustomQuote.sizeName;
    }
    return detailScoreCard?.redBarnComponents?.length > 0 &&
      detailScoreCard.redBarnComponents.filter((item) => item.redBarnCompDetailId.inventoryCompTypeId === 178).length >
        0
      ? detailScoreCard.redBarnComponents.filter((item) => item.redBarnCompDetailId.inventoryCompTypeId === 178)[0]
          .value
      : '';
  }, [dataDetailCustomQuote, detailScoreCard, isCustomQuote, isLoading]);

  const drivetrain = useMemo(() => {
    if (isLoading) return <Skeleton />;
    if (!detailScoreCard?.upgradeComps?.length) return 'None';
    const comp = detailScoreCard.upgradeComps.find((it) => it.name === UpgradeCompsName.drivetrain);
    if (!comp) return 'None';
    return comp.up ? 'Upgraded' : 'Downgraded';
  }, [detailScoreCard, isLoading]);

  const wheels = useMemo(() => {
    if (isLoading) return <Skeleton />;
    if (!detailScoreCard?.upgradeComps?.length) return 'None';
    const comp = detailScoreCard.upgradeComps.find((it) => it.name === UpgradeCompsName.wheels);
    if (!comp) return 'None';
    return comp.up ? 'Upgraded' : 'Downgraded';
  }, [detailScoreCard, isLoading]);
  ...
  useEffect(() => {
    if (dataDetailStandardQuote?.redBarnCustomQuotesId) {
      dispatch(scoreCardAction.getDetailCustomRedBarnQuote(dataDetailStandardQuote.redBarnCustomQuotesId));
    }
  }, [dataDetailStandardQuote, dispatch]);
  ...
  const renderBicycleItem = useCallback(
    (title: string, value?: string | number | React.ReactNode) => {
      return (
        <div className={classes.wrapBicycleInfoItem}>
          <div className={cx(classes.textGrey, classes.mb9, classes.mobileItemTitle)}>{title}</div>
          <div className={cx(classes.textBlack, classes.mobileItemValue)}>{isLoading ? <Skeleton /> : value}</div>
        </div>
      );
    },
    [isLoading],
  ...
  const renderFormInput = useCallback(
          ...
          <div className={classes.formItem}>
            ...
            {isInputPhone ? (
              <FormikTextMask
                ...
                disabled={isCompleted || statusHistoryQuote !== ''}
              />
            ) : (
              <FormikInput
                name={formName}
                ...
                disabled={isCompleted || formName === 'proofName' || statusHistoryQuote !== ''}
              />
            )}
          </div>
        </Col>
    ...
    [handleChange, isCompleted, renderFormTitle, statusHistoryQuote],
  );
  ...
  const renderFormSelect = useCallback(
    ...
    [currentWidthScreen, handleChange, isCompleted, renderFormTitle, statusHistoryQuote],
  );
  ...
  const onSubmit = useCallback(
    (values: FormSummary) => {
      ...
      handleSubmitStepSummary(false, values).then((isComplete) => {
        if (isComplete) setIsOpenModalComplete(true);
      });
    },
  ...
  const printScoreCard = useCallback(async () => {
    ...
    const target = document.getElementById('PrintScoreCardRedBarn');
    if (target) {
  ...
  const handleConverQuote = useCallback(async () => {
    try {
      // await updateStatusQuotes(String(router?.query?.id), 'CONVERTED');
  ...
  return (
    <>
          ...
          return (
            <Form onSubmit={handleSubmit}>
              <div className={classes.wrapStepTwo}>
                  ...
                  <div>
                    ...
                    <div className={classes.wrapBicycleInfo}>
                      ...
                      {renderBicycleItem(
                        'Year',
                        isCustomQuote
                          ? dataDetailCustomQuote?.yearName
                ...
                <Button
                  id="button-print-redbarn-step-summary"
      ...
      <PrintScoreCardRedBarn dataFormStepSummary={dataPrint} />
      ...
      {isOpenModalComplete && (
        <Suspense fallback={null}>
          <ModalComplete
            isOpen={isOpenModalComplete}
            onClose={() => setIsOpenModalComplete(false)}
            onPrint={() => {
