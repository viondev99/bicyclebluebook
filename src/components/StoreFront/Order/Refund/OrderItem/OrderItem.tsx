...
import { DetailOrderModel, AmountCustom } from 'model/store/store-front/order.model';
...
const OrderInfo: FC<Props> = ({ order, errors, amounts, onChangePriceRefund }) => {
  ...
  const renderInputRefund = useCallback(
    (item: LineItemModel) => {
      ...
      const LinkViewRefund = `/store-front/return-detail/mylistings/${item?.master_listing_id}/${order._id}?inventory=${item?.market_listings[0]?.inventory_id}&marketListing=${item?.market_listings[0]?.market_listing_id}`;
      ...
      return AllowRefund ? (
        <div>
          <Input
            ...
            renderPrefix={<img src={images.iconDollar} alt={'Dollar Icon'} className="icon-button22" />}
    ...
    [checkValidateError, findAmountValue, onChangeValueAmounts, order, totalListingRefundedInlineItem],
  );
