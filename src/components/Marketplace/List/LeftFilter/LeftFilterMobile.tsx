...
import icCloseWhite from 'assets/img/marketplace/ic_close_white.svg';
import icCheckWhite from 'assets/img/marketplace/ic_check_white.svg';
...
const LeftFilterMobile: FC<Props> = ({ isViewSales, onRestart, onCloseFilter }) => {
  const [searchValue, setSearchValue] = useState('');
  const [searchFamilyValue, setSearchFamilyValue] = useState<string>('');
  ...
  useEffect(() => {
    dispatch(marketplaceActions.addFilter(query));
  }, [dispatch, query]);
  ...
  const handleSearch = useCallback(
    (newQuery: string | ParsedUrlQueryInput | any) => {
      ...
      const SellerBBB = newQuery?.[SEARCH_TYPE.sellerType];
      if (SellerBBB) {
        if (SellerBBB === BBB_STAFF) {
          router.push({
    ...
    [asPath, isStoreOrSellerPage, onCloseFilter, pathname, router],
  );

  const handleChangePrice = useCallback(
    ([start, end]) => {
      dispatch(
        marketplaceActions.addFilter({ ...leftFilter, [SEARCH_TYPE.startPrice]: start, [SEARCH_TYPE.endPrice]: end }),
      );
    },
    [dispatch, leftFilter],
  );
  ...
  const handleChangeYear = useCallback(
    ([start, end]) => {
      dispatch(
        marketplaceActions.addFilter({ ...leftFilter, [SEARCH_TYPE.startYear]: start, [SEARCH_TYPE.endYear]: end }),
      );
    },
    [dispatch, leftFilter],
  );
  ...
  const handleRemoveAll = useCallback(() => {
    dispatch(marketplaceActions.addFilter());
    handleSearch({});
  }, [dispatch, handleSearch]);

  const handleApply = useCallback(() => {
    handleSearch(leftFilter);
  }, [handleSearch, leftFilter]);

  const onChangeParam = (name: string) => (value: string[] | string) => {
    dispatch(marketplaceActions.addFilter({ ...leftFilter, [name]: value }));
  ...
  const familyWithFiltered: ItemLabelValue[] = useMemo(
    () =>
      listFamily.filter((option: ItemLabelValue) =>
        option.label.toLowerCase().includes(searchFamilyValue.toLowerCase()),
      ),
    [listFamily, searchFamilyValue],
  );
  ...
  const checkStoreName = useMemo(() => {
    return leftFilter?.[SEARCH_TYPE.sellerType] === Roles.ONLINE_STORE.toUpperCase();
  }, [leftFilter]);

  return (
    <div style={{ marginBottom: currentWidthScreen > 1023 && 20 }}>
      <div className={cx('d-none', 'd-lg-block')}>
        <Wishlist disabled={noFilter} isHotBike={isHotBike} />
      </div>
      <div className={cx('d-block', 'd-lg-none')}>
        <FilterPanel title={'Wishlist'} open={false} overflow={false}>
          <Wishlist isMobile disabled={noFilter} isHotBike={isHotBike} />
        </FilterPanel>
      </div>
      {
        <FilterPanel title={'Seller Type'} open={sellType.length > 0}>
          <FilterRadio
            options={listSellerOptions}
            value={leftFilter?.[SEARCH_TYPE.sellerType] ? leftFilter?.[SEARCH_TYPE.sellerType].toString() : sellType}
            onChange={onChangeParam(SEARCH_TYPE.sellerType)}
          />
        </FilterPanel>
      }
      {checkStoreName ? (
        <FilterPanel title={'Store Name'} open={listStoreFrontOptions.length > 0}>
          <FilterCheckboxes
            ...
            values={leftFilter?.[SEARCH_TYPE.sellerType] ? leftFilter[SEARCH_TYPE.storeName] : storeName || []}
      ...
      <FilterPanel title={'Category'} open={listingTypes.length > 0}>
        <FilterCheckboxes
          ...
          values={leftFilter?.[SEARCH_TYPE.listingTypes] ? leftFilter?.[SEARCH_TYPE.listingTypes] : listingTypes}
      ...
      {!isHotBike && (
        <FilterPanel title={'Type of Bike'} open={typeName.length > 0}>
          <FilterCheckboxes
            options={listBikeType}
            values={leftFilter?.[SEARCH_TYPE.typeName] ? leftFilter?.[SEARCH_TYPE.typeName] : typeName}
            onChange={onChangeParam(SEARCH_TYPE.typeName)}
          />
        </FilterPanel>
      )}
      <FilterPanel
        ...
        <FilterCheckboxes
          options={brandWithFiltered}
          values={leftFilter?.[SEARCH_TYPE.brand] ? leftFilter?.[SEARCH_TYPE.brand] : brand}
          onChange={onChangeParam(SEARCH_TYPE.brand)}
        />
      </FilterPanel>
      <FilterPanel
        ...
        headerCollapse={
          <>
            {brand?.length > 0 && (
              <Input
                value={searchFamilyValue}
                onChange={(e) => setSearchFamilyValue(e.target.value)}
                placeholder={'Search'}
                inputSize={'s'}
                style={{ marginBottom: 20 }}
              />
        ...
        <FilterCheckboxes
          options={familyWithFiltered}
          values={leftFilter?.[SEARCH_TYPE.family] ? leftFilter?.[SEARCH_TYPE.family] : family}
          onChange={onChangeParam(SEARCH_TYPE.family)}
        />
      </FilterPanel>
      <FilterPanel
        title={'Model'}
        ...
        <FilterCheckboxes
          options={listModelFilter}
          values={leftFilter?.[SEARCH_TYPE.model] ? leftFilter?.[SEARCH_TYPE.model] : model}
          onChange={onChangeParam(SEARCH_TYPE.model)}
        />
      </FilterPanel>
      ...
      <FilterPanel title={'Condition'} open={condition.length > 0}>
        <FilterCheckboxes
          ...
          values={leftFilter?.[SEARCH_TYPE.condition] ? leftFilter?.[SEARCH_TYPE.condition] : condition}
          onChange={onChangeParam(SEARCH_TYPE.condition)}
        />
      </FilterPanel>
      ...
      <FilterPanel title={'Gender'} open={gender.length > 0}>
        <FilterCheckboxes
          options={listGender}
          values={leftFilter?.[SEARCH_TYPE.gender] ? leftFilter?.[SEARCH_TYPE.gender] : gender}
          onChange={onChangeParam(SEARCH_TYPE.gender)}
        />
      </FilterPanel>
      <FilterPanel title={'Frame Size'} open={sizeName.length > 0}>
        <FilterCheckboxes
          options={listFrameSizes}
          values={leftFilter?.[SEARCH_TYPE.sizeName] ? leftFilter?.[SEARCH_TYPE.sizeName] : sizeName}
          onChange={onChangeParam(SEARCH_TYPE.sizeName)}
        />
      </FilterPanel>
      <FilterPanel title={'Wheel Size'} open={wheelSize.length > 0} overflow={false}>
        <FilterSelectBox
          options={listWheelSize}
          values={leftFilter?.[SEARCH_TYPE.wheelSize] ? leftFilter?.[SEARCH_TYPE.wheelSize] : wheelSize}
          onChange={onChangeParam(SEARCH_TYPE.wheelSize)}
        />
      </FilterPanel>
      <FilterPanel title={'Suspension'} open={suspension.length > 0}>
        <FilterCheckboxes
          options={listSuspension}
          values={leftFilter?.[SEARCH_TYPE.suspension] ? leftFilter?.[SEARCH_TYPE.suspension] : suspension}
      ...
      <FilterPanel title={'Frame Material'} open={frameMaterialName.length > 0}>
        <FilterCheckboxes
          ...
          values={
            leftFilter?.[SEARCH_TYPE.frameMaterialName]
              ? leftFilter?.[SEARCH_TYPE.frameMaterialName]
              : frameMaterialName
          }
      ...
      <FilterPanel title={'Brake Type'} open={brakeTypeName.length > 0}>
        <FilterCheckboxes
          ...
          values={leftFilter?.[SEARCH_TYPE.brakeTypeName] ? leftFilter?.[SEARCH_TYPE.brakeTypeName] : brakeTypeName}
      ...
      <Link scroll={false} shallow={true} replace={true} href={renderPathname}>
        <a onClick={handleRemoveAll} className={cx(classes.clearAllButton, 'd-none', 'd-lg-block')} type="button">
      ...
      <div className={classes.wrapBtnApply}>
        <Button buttonType="danger" className={classes.btnRemove} onClick={handleRemoveAll}>
          Remove All
          <Image src={icCloseWhite} alt="ic_close_white" priority height={16} width={16} unoptimized className="ml-2" />
        </Button>
        <Button buttonType="primary" className={classes.btnRemove} onClick={handleApply}>
          Apply
          <Image src={icCheckWhite} alt="ic_close_white" priority height={16} width={16} unoptimized className="ml-2" />
        </Button>
      </div>
    </div>
  );
...
export default LeftFilterMobile;
