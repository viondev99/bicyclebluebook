...
import classes from './pecificationSection.module.scss';

interface Props {
  bicycleInfo: GetListTradeInBicycleParams;
}

const categorysName = ['Drivetrain', 'Wheels', 'Cockpit', 'Brakes', 'Frameset'];

const FormSpecification: FC<Props> = ({ bicycleInfo }) => {
  ...
  useEffect(() => {
    dispatch(getListTradeInBicycle(bicycleInfo));
  }, [dispatch]);
  ...
  return (
    <>
      <Row className={classes.wrapDetailsContent}>{loading ? renderLoading : renderInfo}</Row>
