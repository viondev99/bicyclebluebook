...
import ListingSoldItem from './ListingSoldItem';
...
function ListingsComponent() {
  ...
  const [params, setParams] = useState<ListingsModel>({
    ...
    size: 10,
    sortField: 'SOLD_TIME',
    statusMarketListing: StatusMarketListing.SOLD,
    sortType: 'DESC',
  });
  ...
  const renderSkeleton = useMemo((): ReactElement => {
    return (
      <>
      ...
      return (
        <>
          ...
          {listings.data?.map((listing: ListingItemModel) => (
            <ListingSoldItem key={listing.postingTime} listing={listing} />
          ))}
