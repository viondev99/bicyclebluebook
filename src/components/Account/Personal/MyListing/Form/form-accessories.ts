...
export interface FormListingPartsAccessoriesValue {
  listingType?: string;
...
export function prepareData(form: FormListingPartsAccessoriesValue, isStorefront?: boolean) {
  const compRequests = [
  ...
  // const emailPaypal =
  //     form.emailPaypal && form.emailPaypal.length !== 0 ? form.emailPaypal.trim() : form.emailPaypal || null;
  ...
  let data: CreateListingPartAccessoriesDraftRequest = {
    listingTitle: form?.listingTitle,
    listingType: 'PART_ACCESSORIES',
    isBuyerPaysSignatureFee: !isStorefront ? null : form?.isBuyerPaysSignatureFee === 'true' ? true : false,
    description: form.description || null,
    emailPaypal: null,
    localPickupShipping: !isStorefront ? true : form.localPickupShipping,
...
export const FormSchema = (isStorefront: boolean) =>
  Yup.object().shape({
    listingTitle: Yup.string().nullable().required(t('common.validate.listingTitle')),
    description: Yup.string().nullable().required(t('common.validate.description')),
    salePrice: Yup.string()
      .nullable()
      .required(t('common.validate.salePrice'))
      .max(10, t('common.validate.salePriceMaxLength')),
...
export const prepareDataUpdateListed = (form: FormListingPartsAccessoriesValue, isStorefront?: boolean) => {
  const compRequests = [
    JSON.parse(form?.frameMaterial),
    JSON.parse(form?.gender),
    JSON.parse(form?.brakeType),
